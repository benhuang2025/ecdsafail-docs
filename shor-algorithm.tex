...
\begin{abstract}
This document lays out both the attack principle and the challenge background as groundwork for further writing, in two parts. \textbf{Part~I (General Principles)} explains \textbf{\emph{how} Shor's algorithm step by step breaks the
...
This part focuses on one thing---\textbf{how Shor's algorithm turns the "classically hard problem" of recovering the private key into a computation a quantum machine can carry out efficiently}. It is an
...
denotes the residues mod $N$. (Reminder: \textbf{the public key is written $Q$, the QFT modulus is written $M$}, so the two no longer share a single letter---this is the only departure from many textbooks' notation.)
...
$\mathbb{F}_p$; \textbf{note that \eqref{eq:padd} contains one division} $(x_2-x_1)^{-1}$---i.e.\ one \emph{modular inverse},
...
on a quantum superposition, \textbf{controllably and reversibly} adding a \emph{classical} point to a curve point. It is this one because the entire attack can be summarized in a single sentence:
\begin{center}
...
\textbf{encoded into the preparation and measurement of a quantum state}. The whole conversion is the following four steps.
...
$r\approx2^{256}$, infeasible. The first step of the conversion is to \textbf{not solve for $k$ directly, but construct a "probe function"}:
\begin{equation}
...
\label{eq:period}
\end{equation}
and more generally $f(a,b)=f(a',b')\iff (a-a')+k(b-b')\equiv0\pmod r$. In other words, \textbf{the private key $k$ has been encoded as the
...
$\tfrac{1}{\sqrt{2^t}}\sum_a\lvert a\rangle$), \textbf{no longer a single $\lvert a\rangle$} (the rigorous state evolution is in \secref{sec:evolve}).
...
With $U_f$ in hand, the core step of the conversion is: \textbf{rather than trying $(a,b)$ one by one, first prepare all $(a,b)$ into a \emph{superposition}, then evaluate $f$
...
obtained), \textbf{yet there is no one-by-one evaluation loop}:
\begin{itemize}[leftmargin=1.4em]
...
\end{itemize}
So the qubit count and gate count throughout are $\mathrm{poly}(t)$, \textbf{with no $2^t$ factor}: some $t$-order qubits carry the computation of $2^{512}$ branches. The exponential
...
\textbf{But it must also be stressed: this is not a "free brute-force search."} This table \emph{cannot be read out}---one measurement collapses to a single random $(a,b)$, and all other branches are lost.
...
on exponentially many inputs, and the periodicity of $k$ \textbf{now physically exists in the structure of this state} (the rigorous ket-by-ket evolution is in \secref{sec:evolve}).
...
(unfolded, with the primitive's location, in \secref{sec:circuit}). It \textbf{almost monopolizes} the whole machine's Toffoli and qubit budget, and is the
...
\label{eq:dbladd}
\end{equation}
where $2^iP$, $2^jQ$ are all \textbf{classical points} ($a,b$ are quantum, but $P,Q$ are known constants, so these multiples can be precomputed classically).
...
\begin{center}
\emph{"controllably adding a classical point to the accumulating point on a superposition"} --- \textbf{is the primitive this challenge optimizes.}
\end{center}
...
arithmetic (constant subtraction, modular inverse, modular multiplication, \dots), overwriting $R$'s coordinates \emph{in place} with those of $R+C$---\textbf{one operand a constant, the other quantum
...
\end{tikzpicture}
\caption{Inside $U_f$ (schematic). The scalar multiplications $aP$, $bQ$ unfold via double-and-add into about $2n$ controlled point additions. \textbf{Every box is the \emph{same}
...
But a quantum circuit is a \emph{fixed unitary}---\textbf{the circuit structure cannot depend on the (superposed) data} (no runtime \texttt{if}), and mid-circuit measurement to decide branches
...
\label{eq:orth}
\end{equation}
i.e.\ \textbf{only when $d\equiv kc$ do the $b$-terms align in phase and add constructively; otherwise they spread uniformly around the unit circle and cancel completely}. So \eqref{eq:s4} simplifies to
\begin{equation}
\lvert\psi_4\rangle=\frac{1}{\sqrt r}\sum_{c=0}^{r-1}\omega_r^{s_0 c}\ \bigl\lvert c\bigr\rangle\,\bigl\lvert kc \bmod r\bigr\rangle .
\label{eq:s5}
\end{equation}
The measured $(c,d)$ therefore \textbf{necessarily satisfies $d\equiv kc\pmod r$}---exactly the readout condition \eqref{eq:readout} previewed in step four of \secref{sec:reduce},
...
\item Not equal $\Rightarrow$ this sample landed on a bad point ($c\equiv 0$, or rounding recovery failed), so \textbf{go back to Step 1 of \secref{sec:steps} and resample},
until verification passes.
\end{itemize}
The probability that one quantum run gives a usable sample is a constant $\Omega(1)$ (the fraction of good $(c,d)$ times the probability $1-1/r$ that $c$ is invertible), so \textbf{only
...
This period is not something \emph{intrinsically grown} out of $Q=kP$; rather, \textbf{we deliberately choose $f(a,b)=aP+bQ$ to manufacture it}. Look at when $f$ repeats
...
back $a$ off by $k$ steps. This "one step of $b$ $=$ $k$ steps of $a$" holds everywhere and repeats regularly---\textbf{the period arises thereby, and its ratio is exactly $k$}.
...
bits), reversibly overwrite target \emph{in place} with the affine sum $\text{target}+\text{offset}$ (on secp256k1). It \textbf{does not include} the QFT,
...
gates, so \textbf{almost all Toffolis come from \hyperref[step:eval]{Step 2}'s reversible evaluation $U_f$}, and the circuit's peak qubits are also reached during $U_f$ (those wide
modular-inverse scratch registers). So the two quantities in the score---executed Toffoli count and peak qubits---\textbf{essentially measure \hyperref[step:eval]{Step 2}},
...
qubits), \emph{not} "$2330$ minus the input registers." Because the score is $\overline{\text{Toffoli}}\times$peak qubits, \textbf{every qubit reclaimed in the modular inverse directly lowers
...
double-and-add of \eqref{eq:dbladd}), so \textbf{any constant-factor reduction on a single point addition multiplies by several hundred along the whole algorithm}. The number industry really cares about---"how big a fault-tolerant machine
...
$\text{Toffoli}\times\text{qubit}$ is equivalent to \emph{directly revising down} this threat estimate. And this estimate is precisely the quantitative basis for the NIST post-quantum
standardization process and for the ECDSA-deprecation timelines in finance / blockchain / TLS and other industries.

\medskip
And within this space--time volume to be minimized, \emph{which part} has the most optimization room? The answer is highly concentrated---the next section explains.
...
\textbf{the optimization history of this circuit is essentially the optimization history of the modular inverse (GCD)}. There are four reasons, and they compound:

\paragraph{(1) It is the only iterative big-cost component in point addition.}
In one point addition (\eqref{eq:padd}), addition / subtraction / multiplication / squaring are all \emph{branch-free, single-pass} circuits, and the adder (Gidney-style) and
schoolbook / Karatsuba multiplication are already optimized near their lower bounds, with fixed structure. Only the modular inverse has \emph{no} single-pass algorithm; it can only iterate GCD for
...
\textbf{the Toffolis of a single point addition come mainly from the GCD loop body}.
...
\item \textbf{Qubit axis}: Kaliski must maintain several $n$-bit state registers ($u,v,r,s$ and the like) plus scratch at once, and \textbf{the peak width of the whole
...
factor pressed down here \textbf{multiplies by several hundred along the whole algorithm}. Savings on other components are additive; savings on the GCD are multiplicative.

\paragraph{(4) It is the only component with algorithmic freedom left.}
Since the adder / multiplication / squaring are all structurally fixed and hugging their lower bounds, the place in one point addition where real trade-offs remain---\emph{which modular-inverse
algorithm to choose and how to organize its iterations}---is almost entirely concentrated in the GCD. Addition and multiplication have little optimization room left, whereas the modular inverse is still
an open design problem. That is, "the space still to be pressed down" objectively lies in the modular inverse; focusing on it is not a preference but the optimal leverage point dictated by the circuit structure.

\medskip
In a word: minimizing $\text{Toffoli}\times\text{qubit}$ is nearly equivalent to reducing the cost of the modular inverse---the GCD is the \textbf{cost
center, the amplifier, and the sole locus of algorithmic freedom} of a single point addition. This is also why, on this circuit, every advance that pushes cost to a new low ultimately traces back to the same place:
...
\item \textbf{The rest of the arithmetic} (modular multiplication / squaring / addition / comparison, essentially already at the lower bound) --- \code{src/point_add/arith/} and \code{gidney.rs},
